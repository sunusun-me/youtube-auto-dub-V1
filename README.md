...
## ⚠️ 安全须知与免责声明 (Security & Disclaimer)

> 本工具仅供**个人学习与技术研究**使用。请严格遵守你所在地区关于版权、YouTube 服务条款（ToS）及数据处理的相关法律法规。因不当使用（包括但不限于批量下载、再分发受版权保护内容、绕过平台限制）导致的任何账号封禁、法律纠纷或损失，均由使用者自行承担，作者与本项目不提供任何担保或背书。

**关于 Cookie 与凭据（重要）：**

- 本项目**绝不**将任何 `cookies.txt`、账号凭据、`.venv/` 虚拟环境目录提交进版本库（这些已被加入 `.gitignore`）。
- `cookies.txt` 仅用于本地 `yt-dlp` 绕过登录/年龄限制，**必须由使用者自行从浏览器导出**，且应保存在仓库目录之外、不被 Git 追踪的位置（如 `~/.config/yt-dlp/cookies.txt`），切勿上传至任何公开仓库或聊天工具。
- 一旦凭据文件意外被提交或推送，请**立即作废相关会话**（改密码 / 注销所有设备），再将其从历史中彻底抹除——泄露的会话令牌在删除文件后仍可被旧 commit 访问，直至会话失效。
- 请勿将 GitHub、Google、Microsoft 等平台的**登录会话 Cookie 或 Personal Access Token** 放入本仓库。

**关于第三方接口：**

- 翻译默认走 Google Translate 非官方 RPC、TTS 走 Edge TTS 免费接口，二者均为非公开授权调用方式，随时可能因平台策略调整而失效或限流。
- 生产/长期用途建议改用官方 API（如 Google Cloud Translation、Azure TTS）或本地开源 TTS，并为本项目的调用做好失败降级处理。
...
> **Python 版本要求**：本项目已在 **Python 3.12** 上验证通过（Apple Silicon 推荐）。请务必使用 3.10+，3.9 及以下可能因 `onnxruntime` / `faster-whisper` 依赖而失败。
...
⚠️ **Apple Silicon (M1/M2/M3/M4) 用户必读**：`requirements.txt` 中锁定的 `torch` / `onnxruntime` 等是跨平台快照，**直接 `pip install -r requirements.txt` 在 macOS 上可能装到不兼容的构建**。请改用官方 macOS 包（自动启用 Metal / CoreML 加速）：

Bash

```
# 在已激活的 .venv 中，优先安装 Apple Silicon 版 PyTorch（替换锁定的 x86 版）
pip install --upgrade "torch==2.12.1" "torchaudio==2.11.0" "torchvision==0.27.1" \
  --index-url https://download.pytorch.org/whl/cpu
# onnxruntime 在 macOS 使用 CPU 版即可（已随 requirements 安装，无需特别处理）
```
...
> **一键封装脚本 `dub.sh`**（仓库根目录，需先完成 `初始化生产力环境` 创建 `.venv`）：
>
> ```bash
> ./dub.sh <YouTube-URL>            # 中文配音 + 中英双语字幕（源语自动识别）
> ./dub.sh <URL> en                # 英文字幕 + 保留原声（源为 en 时自动跳过配音）
> ./dub.sh <URL> cn                # 中文字幕 + 中文配音
> ./dub.sh <URL> cn medium         # 第三参数指定 Whisper 模型 (tiny/base/small/medium, 默认 small)
> VOICE=zh-CN-YunjianNeural ./dub.sh <URL>   # 环境变量换音色（男声）
> ```
...
## 🧹 磁盘与缓存清理策略 (Disk & Cache)

长时间运行会累积媒体文件，请注意定期清理，避免撑爆磁盘：

| 目录 | 作用 | 是否自动清理 | 建议 |
| --- | --- | --- | --- |
| `output/` | 最终成品视频 | 否（持久保留） | 按需归档/删除，单个长视频可达数百 MB～数 GB |
| `.cache/` | 视频流缓存池（避免重复下载） | 否 | 可随时整体删除以释放空间，下次运行会重新下载 |
| `temp/` | 单次运行的切片/中间音频交换区 | **是（单次运行后自动清空）** | 异常中断后可手动 `rm -rf temp/*` |

清理命令示例：

```bash
# 释放缓存与临时区（不影响已生成的 output）
rm -rf .cache/* temp/*

# 查看各目录占用
du -sh output .cache temp
```
